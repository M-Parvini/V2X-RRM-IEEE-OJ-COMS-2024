...
class Agent:
    ...
    def choose_action(self, observation, option):
        ...
        with torch.no_grad():
            ...
            if rnd < self.epsilon:
                continuous_acts = torch.tensor(np.random.uniform(0, 1, self.n_actions)).to(self.actor_model.device)
            else:
            ...
            if option == 'train':
                continuous_acts = continuous_acts + T.tensor(np.random.normal(scale=self.noise), dtype=T.float).\
                    to(self.actor_model.device)

            continuous_acts = T.clamp(continuous_acts, 0, 1)
    ...
    def learn(self):
        ...
        target_actions = target_actions + \
                         T.clamp(T.tensor(np.random.normal(scale=0.06)), -0.1, 0.1)
        target_actions = T.clamp(target_actions, 0, 1)

        critic1_value_ = self.target_critic1_model.forward(states_, target_actions)
        critic2_value_ = self.target_critic2_model.forward(states_, target_actions)

        critic1_value = self.critic1_model.forward(states, actions)
        critic2_value = self.critic2_model.forward(states, actions)

        critic1_value_[done] = 0.0
        critic2_value_[done] = 0.0
        critic1_value_ = critic1_value_.view(-1)
        critic2_value_ = critic2_value_.view(-1)

        critic_value_ = T.min(critic1_value_, critic2_value_)

        target = rewards + self.gamma * critic_value_
        target = target.view(self.batch_size, 1)

        self.critic1_model.optimizer.zero_grad()
        self.critic2_model.optimizer.zero_grad()

        critic1_loss = F.mse_loss(target, critic1_value)
        critic2_loss = F.mse_loss(target, critic2_value)

        critic_loss = critic1_loss + critic2_loss
        critic_loss.backward()
        self.critic1_model.optimizer.step()
        self.critic2_model.optimizer.step()
